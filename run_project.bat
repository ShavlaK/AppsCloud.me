@echo off
REM Скрипт быстрого запуска проекта Price Updater для Windows
REM Автоматически скачивает проект, настраивает окружение и запускает приложение

setlocal enabledelayedexpansion

echo ==========================================
echo   ЗАПУСК PRICE UPDATER (Windows)
echo ==========================================

set REPO_URL=%REPO_URL:~%https://github.com/YOUR_USERNAME/YOUR_REPO.git%
set PROJECT_DIR=price-updater
...
REM Скачивание портативного Python (используем embeddable package)
...
if not exist "config.json" (
    echo [INFO] Копирование примера конфигурации...
...
%PYTHON_CMD% -c "from price_database import init_db; init_db()" 2>nul || echo [WARN] Не удалось инициализировать БД
